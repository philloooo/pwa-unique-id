...
What globally identifies a PWA is not defined in PWA specifications. This is an exploration document that describes and compares all the options for PWA unique identifiers, and makes a recommendation about the optimal solution. A separate design doc will be created later.
...
[migration](#migration) section below for details here. This means adding implementation and operational costs for the specific user agent.
...
[Migration](#migration) will have to be performed to support existing apps for options that do not use manifest\_url as the identifier.
...
[Migration](#migration) will have to be performed to support existing apps for options that do not use start\_url as the identifier.
...
*   Chrome Desktop - possible
*   Firefox Desktop - possible
*   Chrome Android - not possible
...
*   Chrome Desktop - not possible
*   Firefox Desktop - not possible
*   Chrome Android - possible
...
*   [Migration](#migration) needs to be done for existing browser implementations that don’t use the default identifier to index PWAs. 
...
*   Align with the existing situation of desktop apps. No migration needs to be done for 
[S2 - installed desktop apps. ](#scenario-2)
...
*   [S1: installed apps on Android](#scenario-1) need to handle [migration](#migration).
...
*   [S5: Apps that need to change start\_url](#scenario-5) on **desktop** can perform explicit [app initiated migration](#app-initiated-migration) by specifying both the `id` and a `legacy_start_url` field, or they can just set the `id` to `start_url - origin` to keep the existing identity.
*   [S4: Apps that need to change manifest\_url](#scenario-4), [S6: Manifests with data URL](#scenario-6) on **Android** will perform explicit [migration](#migration) by specifying both the `id` and a `legacy_manifest_url` field. Or if the manifest is under the same origin, they can set the `id` to `manifest_url - origin` to keep the existing identity.
*   For [S3: Apps that are installed on both Android and desktop](#scenario-3), they will set both `id`, `legacy_manifest_url` and `legacy_start_url` to migrate to a consistent id. User agents will look up with the legacy field that matches with their previous implementation to find existing apps to migrate.
...
*   [S2: installed apps on desktop](#scenario-2) need to handle [migration](#migration).
...
*   [S1: installed apps on Android](#scenario-1), [S2: installed apps on desktop](#scenario-2), [S3: Apps that are installed on both Android and desktop](#scenario-3) needs to handle [migration](#migration).
...
*   No existing browser implementation uses the scope to key web apps. [S1: installed apps on Android](#scenario-1), [S2: installed apps on desktop](#scenario-2), [S3: Apps that are installed on both Android and desktop](#scenario-3) needs to handle [migration](#migration).
...
3. Apps that already exist that want to use the ID field will specify both an ID field and a **legacy\_manifest\_url** field. This instructs user agents to look up installed apps by legacy\_manifest\_url to do a **backend** **migration** to the new ID scheme.
...
2. New apps with an ID field specified will use the new ID.
3. Apps that already exist that want to use the ID field will specify both an ID field and **legacy\_start\_url** field. This instructs user agents to look up installed apps by legacy\_start\_url to do a **backend** **migration** to a new ID scheme.

Alternatively, we can have a single **legacy\_ids** array field, which can include any old start URLs and manifest URLs.

Pros:



*   One less field to add.
...
*   Not explicit - eg: Chrome desktop will treat any IDs in this field as legacy start URLs to mint IDs to match with existing IDs.
*   Need to handle edge cases of finding multiple installed apps with the array of IDs, if app developers had a start\_url\_v1 and start\_url\_v2, do we help to migrate both apps to the new single app?

When legacy\_start\_url/legacy\_manfiest\_url is presented, exactly how to migrate will be an implementation detail of the user agent.

For Chrome desktop, these are the possible ways to do the migrate:

Plan A:

When a manifest is loaded, look up existing apps with id = &lt;new id>/&lt;legacy\_start\_url>. 
...
When a manifest is loaded, look up existing apps with id = &lt;new id> or &lt;legacy\_start\_url>. 
